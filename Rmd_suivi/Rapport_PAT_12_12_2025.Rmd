...
```{r}
# on ne choisit pas le nombre de mots caractéristiques = 7 
...
# ON CHOISIT LE NOMBRE DE MOTS CARACTERISTIQUE ET LE SEUIL DU PARAMETRE DE POIDS W 
...
# création des modèles 
# for (i in 1:nb_lda){
#   modeles[[i]] <-  lda.model(k = k, seeds[i])
# }

# save(modeles, file = "data/modeles.RData")
...
```{r}
# ⚠️⚠️⚠️⚠️avec la nouvelle fonctipn topic.extraction2 🐲 : 
liste_mots_modeles = topic.extraction2(modeles, nb.frex = 7) # on créé la liste avec les mots de chaque lda : n objets, contenant chacun k éléments (9 ici)

words <- unique(unlist(liste_mots_modeles))# récupére la liste unique des mots de toutes les lda

# save(liste_mots_modeles, file = "data/liste_mots_modeles.RData")
...
for (i in 1:nb_lda) {
  for (j in 1:k) {
    ...
    words_in_topic <- liste_mots_modeles[[i]][[j]]
...
```{r}
mfa.df.final <- mfa.df
mfa.df.final$freq <- apply(mfa.df, 1, sum)

# avec freq >= 2
mfa.df.final <- subset(mfa.df.final, freq>=2)

mfa.df.final <- mfa.df.final[,-ncol(mfa.df.final)] # supprimer la colonne fréquence 
...
```{r}
res.hcpc <- HCPC(res = res.mfa, nb.clust = -1) # nb optimal de classes
...
topics_fortifies_cluster <- data.frame(cbind(word = rownames(mfa.df.final),
                           clust = res.hcpc$data.clust$clust, 
                           dim1 = res.mfa$ind$coord[,1], 
                           dim2 = res.mfa$ind$coord[,2]))

str(topics_fortifies_cluster)
topics_fortifies_cluster$clust <- as.factor(topics_fortifies_cluster$clust)
topics_fortifies_cluster$word <- as.factor((topics_fortifies_cluster$word))
topics_fortifies_cluster$dim1 <- as.numeric(topics_fortifies_cluster$dim1)
topics_fortifies_cluster$dim2 <- as.numeric(topics_fortifies_cluster$dim2)

head(topics_fortifies_cluster)

# save(topics_fortifies_cluster, file = "data/topics_fortifies_cluster.RData")
```
...
mots.clust <- tapply(
  topics_fortifies_cluster$word,     # Le vecteur à appliquer la fonction (les Mots)
  topics_fortifies_cluster$clust, # Le facteur de regroupement (les Clusters)
  paste,                # La fonction à appliquer
  collapse = ", ")       # L

# interprétation avec naileR

cat(gemini_generate(nail_textual(dataset = topics_fortifies_cluster[,1:2], num.var = 2, num.text = 1,
...
theta_all <- data.frame(cbind(modeles[[1]]$theta, modeles[[2]]$theta, modeles[[3]]$theta, modeles[[4]]$theta, modeles[[5]]$theta, modeles[[6]]$theta, modeles[[7]]$theta, modeles[[8]]$theta, modeles[[9]]$theta , modeles[[10]]$theta))
...
Une autre approche pour obtenir les compositions des PAT avec les formes fortes et d'assigner chaque topic de lda à une forme forte en calculant la distance avec les centroides et d'assigner chaque topic de lda à la forme forte avec laquelle il a la plus petite distance.

```{r}
#Determination de l'association entre forme forte et topics de lda

#On recupère les coordonnées de nos topics de LDA sur l'AFM que nous avons fait
freq <- as.data.frame(res.mfa$freq$coord)
freq$clust <- NA

#On récupère les coordonnées des PAT et on rajoute le cluster auquel ils sont associés pour calculer les centroides
...
#On réordonne notre tableau pour avoir les 5 dimensions et le cluster pour ensuite calculer la distance
...
On prépare notre premier tableau de composition (fréquence sur le comptage simple des termes forts) de nos PAT en forme forte
...
Pour réaliser la typologie à partir de ces vecteurs de proportions en 6 topics fortifiés, on va utiliser ces deux approches méthodologiques et réaliser une AFM 
```{r}
...
### Typologie de PAT sur vecteurs fortifiés 

```{r}
# On va faire la classification sur l'un de ces tableaux de données
# ⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️  choix du nombre de dimensions conservées !!!!!!ne n donnera pas le même nombre de clusters à la CAH 
...
Nos clusters sont totalement cohérents avec les clusters précédemment formés avec la méthode "basique". 
Ils surreprésentent les mêmes thématiques et en opposition sous-représentent également les mêmes thématiques.



Illustration des clusters  : voir le doc Typologie_PAT_id_thématiques 
Code illustratif à ne pas 
```{r}
...
# 
# 
# # on refait ensuite l'AFC avec des variables illustratives supplémentaires
# 
...
# 
# 
# Graphique des clusters amélioré : 
# 
# ```{r}
# ##############################
...
# 
# # et on combine avec les coordonnées sur les axes 1 et 2
...
#   theme_bw() +
#   
#   ggtitle ("Typologie des PAT construite sur leur proportion en topics fortifiés dans leur description", 
