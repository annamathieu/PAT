...
d <- as.matrix(dist(combined))

nA <- nrow(freq)
nB <- nrow(centre)

D <- d[1:nA, (nA + 1):(nA + nB)]
colnames(D) <- c("Gouvernance","Education alim","Territoires","Environnement","Secteur agricole","Economie alimentaire")


class <- apply(D,1, which.min)
...
On prépare notre premier tableau de composition de nos PAT en forme forte
...
colnames(mfa_0) <- c("Gouvernance_0","Education_alim_0","Territoire_0","Environnement_0","Secteur_agri_0","Economie_alimentaire_0")
colnames(theta_resume2) <- paste0(colnames(theta_resume_unique),"_2")
...
res.afc.theta.1 <- FactoMineR::CA(theta_resume_unique, ncp = 3)
barplot(res.afc.theta.1$eig[,2])

res.hcpc.combine <- HCPC(res.afc.theta.1,nb.clust=3)
...
On va faire la classification avec l'autre méthode utilisant les centroides. 
...
df_illus_freq_afc <-  left_join(data.frame(cbind(num = rownames(theta_resume_unique), theta_resume_unique)),
...
res.afc.theta.1.illus <- FactoMineR::CA(df_illus_freq_afc,
                                        quali.sup = quali.sup,
...
res.hcpc.test <- HCPC(res.afc.theta.1.illus, nb.clust = -1)
