...
plot <- as.data.frame(rbind(ca.luc$row.sup$coord,ca.luc$col$coord))
plot$groupe <- c(rownames(plot)[1:9],rep("a",6))
plot$nom <- rownames(plot)

ggplot(data = plot[1:9,], aes(x = `Dim 1`, y = `Dim 2`, col = groupe)) +
  geom_text(
    data = plot[1:9,],
    aes(label = nom),
    position = position_dodge(width = 0.3)
  ) +
  geom_point(data=plot[10:15,],aes(x = `Dim 1`, y = `Dim 2`),col = "black")+
  geom_text(data=plot[10:15,],aes(label = nom),position = position_dodge(width = 0.3),col="black")+
  theme_bw()
...
#Représentation de l'évolution temporelle des compositions des descriptions abordées par les PAT
```{r}
load("data/df_textes.RData")

#On représente l'évolution temporelle des descriptions des PAT

res.ca <- CA(theta_resume_unique)
...
coord.PAT <- res.ca$row$coord
coord.PAT <- as.data.frame(coord.PAT)
rownames(coord.PAT) <- gsub("text","",df_textes$doc)

graph <- coord.PAT%>%
  rownames_to_column("Numero") %>%
  left_join(
    pat2025 %>%
      rownames_to_column("Numero") %>%
      select(Numero,annee_de_signature_de_la_convention),
    by = "Numero"
  ) %>%
  column_to_rownames("Numero")
...
graph <- graph %>% 
  ...
  mutate(n = n(),
         annee_de_signature_de_la_convention = paste0(annee_de_signature_de_la_convention, " (n = ", n,")"))

rownames(plot)[10:15] <- c("Logistique-Commercialisation","Education et restauration collective","Environnement-Santé","Gouvernance","Secteur agri","Territoires")
...
graph %>% 
  ...
  summarise(Dim.1 = mean(`Dim 1`),
            ...
            Dim.4 = mean(`Dim 4`),
            Dim.5 = mean(`Dim 5`)) %>% 
  mutate(alpha = c(rep(1,8),0.1)) %>% 
  ggplot(aes(x=Dim.1,y=Dim.2,col=as.factor(annee_de_signature_de_la_convention))) +
  geom_hline(yintercept = 0,col="black") +
  geom_vline(xintercept = 0,col="black") +
  geom_point(size = 2.8, alpha = alpha)+
  geom_path(
    aes(group = 1),
    arrow =  arrow(type = "open", angle = 30, length = unit(0.12, "inches")),
    linewidth = 1
  ) +
  theme_bw() +
  guides(col=guide_legend(title="Année de signature de la convention"))+
  geom_point(data = plot[10:15,], aes(x = `Dim 1`, y = `Dim 2`),col="black") +
  geom_text(data = plot[10:15,], aes(x = `Dim 1`-0.1, y = `Dim 2`-0.05,label=nom),col="black",size=4.9)+
  ggtitle("Evolution temporelle du profil moyen des PAT (2017-2025)",subtitle = "Les points noirs représententent les centroides de nos formes fortes dans un même espace latent") +
