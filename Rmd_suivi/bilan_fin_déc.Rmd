...
### Illustration des clusters formés
...
compte_theme (intensite systémique => nb de thème sur lesquels le PAT est engagé)
...
df_clust_ill <- pat2025[,c("regions", "departement", "nombre_dhabitants_permanents_du_territoire",
 ...
 "repartition_des_exploitations_par_otex_toutes_exploitations",
 "repartition_des_exploitations_par_otex_grandes_cultures",
 "repartition_des_exploitations_par_otex_maraichage_ou_horticulture",
 "repartition_des_exploitations_par_otex_viticulture",
 "repartition_des_exploitations_par_otex_cultures_fruitieres_ou_autres_cultures_permanentes",
...
df_clust_ill$num <- rownames(df_clust_ill)

# on récupère la colonne compte_themes depuis A2
compte_theme <- data.frame(num = rownames(A2), compte_theme = A2$compte_theme )

df_clust_ill <- left_join(df_clust_ill, compte_theme, by = "num")
...
# il faut prendre le tableau disjonctif des thèmes abordés et non comptes_themes



df_ca_ill<- df.ca
df_ca_ill$num = rownames(df.ca)

df_ca_ill <- left_join(df_ca_ill, df_clust_ill, by = "num" )

# save(df_ca_ill, file = "data/df_ca_ill.RData")
...
```{r}
load(file="data/df_ca_ill.RData")
summary(df_ca_ill)
names(df_ca_ill)


# passer les colonnes nécessaires en quali 
df_ca_ill[, c(8:9,28:29,48) ] <- lapply(df_ca_ill[, c(8:9,28:29,48 )], factor)
summary(df_ca_ill)
...
cols <- c(10:16, 18, 24:27, 30, 32:43, 45,47)

# supprimer tous les espaces dans les colonnes quanti 
df_ca_ill[, cols] <- lapply(df_ca_ill[, cols ], function(x) gsub(pattern = " ", replacement = "", x))
...
df_ca_ill[, cols ] <- lapply(df_ca_ill[,cols ], as.numeric)


# supprimer la colonne "repartition_des_exploitations_par_otex_toutes_exploitations " => information redondante 
df_ca_ill <- df_ca_ill[,-15] 
...
# on resave le RData 
# save(df_ca_ill, file = "data/sdf_ca_ill.RData")
...
```{r}
df_ca_ill_f <- df_ca_ill[,-7] # on retire la colonne num 

quanti.sup <- which(sapply(df_ca_ill_f, is.numeric))
quanti.sup <- quanti.sup[-c(1:6)] # on retire les colonnes 1 à 6 qui seront les colonnes actives 
quali.sup  <- which(sapply(df_ca_ill_f, function(x) is.factor(x) || is.character(x)))

res.ca.ill <- FactoMineR::CA(df_ca_ill_f, quanti.sup = quanti.sup, quali.sup =  quali.sup  )
plot.CA(res.ca.ill)
