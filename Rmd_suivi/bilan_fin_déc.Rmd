...
load(file = "data/topics_fortifies_cluster.RData")     # coordonnéees des mots = formes fortes 
...
```{r}
# à faire 
# + description des PAT (à demander)=> cf Typologie_PAT_id_thematiques => partie Luc
```
...
topics_fortifies_cluster <- topics_fortifies_cluster %>% arrange(by = word) # ⚠ important avant le merge de la colonnes clust !!
  
df_resultat$cluster <- topics_fortifies_cluster$clust
...
# Comparaison approches de classification 
...
res.hcpc.greenacre <- HCPC(res.pca, nb.clust = -1)
```

Description des clusters 
```{r}
res.hcpc.greenacre$desc.var
...
## AFC sur table de comptage FREQUENCE SIMPLE
...
Description des clusters 
```{r}
res.hcpc.ca$desc.var$`1` # territoire, secteur agri
res.hcpc.ca$desc.var$`2` # education alim++ , env++ , eco alim ++
res.hcpc.ca$desc.var$`3` # gouvernance 
```

```{r}
# ou : 
res.catdes <- catdes(donnee = res.hcpc.ca$data.clust, num.var = 7, proba = 0.10 )
...
```{r}
# cluster 1 : sur représentation de secteur agri + territoire 
# indice para :  270       184       398        14       452 
subset(df.ca, rownames(df.ca) %in% c("270", "184", "398", "14", "452"))
```


```{r}
# cluster 2 : env, justice sociale-éducation , eco alim
# para : 252         6       453       385       241
subset(df.ca, rownames(df.ca) %in% c("252", "6", "453", "385", "241"))

```


```{r}
# cluster 3 : gouvernance 
# para :  372       212       283       260       110 
subset(df.ca, rownames(df.ca) %in% c("372", "212", "283", "260", "110"))


```
On est allés lire les descriptions des individus les plus proches des centroides des clusters. Ils représentent en majorité assez bien leurs clusters.


### Ajout à pat.df des colonnes de fréquence + clust
```{r}
# temp2 <- df
# temp2$num = rownames(temp2)
# pat.df$num = rownames(pat.df)
# pat.df <- left_join(pat.df, temp2, by="num" )
# colnames(pat.df)[4:9]<- c("1:Gouvernance", "2:Education/Justice Sociale", "3:Territoires", "4:Environnement", "5:Secteur Agricole", "6:Economie alimentaire")
# 
# temp.clust<- data.frame(cbind(num = rownames(res.hcpc.ca$data.clust), clust_afc_freq = res.hcpc.ca$data.clust$clust))
# pat.df <- left_join(pat.df,temp.clust , by = "num")

# save(pat.df, file = "data/pat.df.RData")
```

### Compréhension des clusters : box plot de la répartition des thématiques par la méthode de la fréquence simple 

```{r}
temp3 <- data.frame(cbind(df, clust = res.hcpc.ca$data.clust$clust)) # on combine les % de topics et les clusters attribués aux PAT 


# puis on calcule les stat descriptives par clusters : 
long_df <- temp3 %>%
  pivot_longer(
    cols = -clust,          
    names_to = "Theme",
...
# vérification des valeurs des stats descriptives du boxplot
# temp3 %>%
#   group_by(clust) %>% 
#   summarise(min = min(Economie.alimentaire), median = median(Economie.alimentaire), mean = mean(Economie.alimentaire), max = max(Economie.alimentaire))
...
plot <- long_df %>%  ggplot(aes(x = Theme, y = Valeur, fill = clust)) +

  geom_boxplot() +

  scale_fill_manual(values = c("gold1", "springgreen4", "royalblue")) +
  theme_minimal() +
  labs(title = "Distribution par groupe",
       x = "Groupe",
       y = "Valeur") +
...
On voit finalement que les différences de composition des topics entre PAT des différents clusters ne sont pas tellement marquées


## Comparaison des deux classifications (Greenacre - AFC fréquence simple)
...
# Illustration des typologies 

Sélection des variables illustratives : 

Les variables que l'on souhaite ajouter pour illustrer nos clusters sont :

nombre_dexploitations_agricoles
pourcentage_de_la_sau
nombre_demplois_dans_la_transformation
nombre_denseignes_de_distribution
nombre_denseignes_de_distribution_de_gms

repartition_des_exploitations_par_otex_grandes_cultures
repartition_des_exploitations_par_otex_maraichage_ou_horticulture
repartition_des_exploitations_par_otex_viticulture
repartition_des_exploitations_par_otex_cultures_fruitieres_ou_autres_cultures_permanentes
repartition_des_exploitations_par_otex_bovins_lait
repartition_des_exploitations_par_otex_bovins_viande
repartition_des_exploitations_par_otex_bovins_mixtes
repartition_des_exploitations_par_otex_ovins_et_ou_caprins_et_ou_autres_herbivores
repartition_des_exploitations_par_otex_porcins_et_ou_volailles
repartition_des_exploitations_par_otex_polyculture_et_ou_polyelevage_et_ou_non_classe

pourcentage_de_la_sau_en_bio
emission_de_ges
pourcentage_de_la_surface_des_prairies_permanentes
pourcentage_de_la_surface_des_cultures_de_legumineuses
metres_de_haie_par_hectare_de_sau
m3_deau_utilise_pour_lirragation
evolution_de_lirrigation_depuis_5_ans
milligrammes_de_nitrates_par_litre_deau

regions
departement
taux_de_pauvrete_du_territoire
taux_de_chomage_annuel_moyen
revenu_median
...
nombre_de_cantines_inscrites
nombre_de_captage_prioritaire_grenelle_dans_le_perimetre
microgrammes_de_produits_phytopharmaceutiques_par_litre_deau
nombre_dhabitants_permanents_du_territoire
surface_des_communes_du_perimetre_en_km2
densite_de_population_hab_km2
part_de_la_surface_du_perimetre_artificialisee


On va aussi ajouter : 

annee_de_premiere_labellisation 
niveau_de_labellisation 
le tableau disjonctif des thèmes sur lesquels les PAT sont impliqués (intensite systémique => nb de thème sur lesquels le PAT est engagé)


Création du tableau de données 
```{r}
load(file = "data/pat2025.RData")
load(file = "data/themes_abordes.RData")

df_clust_ill <- pat2025[,c("regions", "departement", "nombre_dhabitants_permanents_du_territoire",
 "densite_de_population_hab_km2", "surface_du_territoire_artificialisee",
 "revenu_median", "pourcentage_de_chomage",
 "repartition_des_exploitations_par_otex_grandes_cultures",
 "repartition_des_exploitations_par_otex_maraichage_ou_horticulture",
 "repartition_des_exploitations_par_otex_viticulture",
 "repartition_des_exploitations_par_otex_cultures_fruitieres_ou_autres_cultures_permanentes",
 ...
 "annee_de_signature_de_la_convention",
  "surface_des_communes_du_territoire_en_km2" ,
 "nombre_dassociations_luttant_contre_la_precarite_alimentaire",
 "nombre_de_restaurants_collectifs_sur_le_territoire_qui_ont_fait_leur_teledeclaration_sur_ma_cantine",
 "nombre_de_cantines_inscrites",
 "nombre_de_captage_prioritaire_grenelle_dans_le_perimetre",
...
summary(df_clust_ill)

# passer les colonnes nécessaires en quali 
df_clust_ill[, c(1:2,20:21) ] <- lapply(df_clust_ill[, c(1:2,20:21)], factor)

# passer les colonnes nécessaires en quanti 
cols <- c(3:8,10,16:19,22,24:35,37,39)

# supprimer tous les espaces dans les colonnes quanti 
df_clust_ill[, cols] <- lapply(df_clust_ill[, cols ], function(x) gsub(pattern = " ", replacement = "", x))

# remplacer virgules par des points 
df_clust_ill[, cols] <- lapply(df_clust_ill[, cols ], function(x) gsub(pattern = ",", replacement = ".", x))


# transformer les caractères en variables numériques  
df_clust_ill[, cols ] <- lapply(df_clust_ill[,cols ], as.numeric)
...
```{r}
# ⚠️ il faut prendre le tableau disjonctif des thèmes abordés et non comptes_themes
# JOIN AVEC THEMES ABORDES 
df_clust_ill <- left_join(data.frame(cbind(df_clust_ill, num = rownames(df_clust_ill))), 
                          data.frame(cbind(themes_abordes, num = rownames(themes_abordes)))
                          , by = "num")
...
# save(df_clust_ill, file = "data/df_clust_ill.RData")
load(file = "data/df_clust_ill.RData")
```


## UTILISATION DES VARIABLES ILLUSTRATIVES POUR ILLUSTRER LES CLUSTERS CONSTRUITS AVEC FREQUENCES SIMPLE SUR LES OCCURENCES DES TERMES FORTIFIES 

On refait l'AFC avec les variables illustratives 
```{r}
# # création du jeu de données combinant le df.ca et df_clust_ill 
# df.afc.bas.illus <- left_join(data.frame(cbind(df.ca, num = rownames(df.ca))), df_clust_ill  , by = "num")
# 
# # afc avec variables supp quali et quanti 
# quanti.sup <- which(sapply(df.afc.bas.illus, is.numeric))
# quanti.sup <- as.numeric(quanti.sup[-c(1:6)]) # on retire les colonnes 1 à 6 qui seront les colonnes actives
# quali.sup  <- as.numeric(which(sapply(df.afc.bas.illus, function(x) is.factor(x) || is.character(x))))
# 
# 
# # PB NA ; on VA SUPPRIMER LES COLONNES AVEC DES NA 
# temp.df <- df.afc.bas.illus %>%
#   select(
#     -num, 
#     -taux_de_pauvrete_du_territoire,
#     -emission_de_ges,
...
#   )
# 
# quanti.sup <- which(sapply(temp.df, is.numeric))
# quanti.sup <- as.numeric(quanti.sup[-c(1:6)]) # on retire les colonnes 1 à 6 qui seront les colonnes actives
# quali.sup  <- as.numeric(which(sapply(temp.df, function(x) is.factor(x) || is.character(x))))
# 
# # quali.sup <- c(7,8,9,27,28)
# # quanti.sup <- seq(7,46)[!(seq(7,46)) %in% quali.sup]
# 
# 
# res.afc.ill <- FactoMineR::CA(temp.df, quanti.sup = quanti.sup , quali.sup =  quali.sup  )
# plot.CA(res.afc.ill)
# 
# 
# colSums(is.na(df.afc.bas.illus))
