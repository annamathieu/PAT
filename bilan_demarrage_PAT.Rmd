...
La collecte de données a lieu deux fois par an. Nous avons accès aux deux collectes de données des années 2024 et 2025. 

Il y a un fichier Excel pour la description du PAT (2 par an sur 2 ans => 4 ), et un fichier par axe thématique (il y en a 9 au total, donc a 9 * 2 années * 2 fichiers par an = 36 fichiers). 
Chaque ligne de chaque fichier correspond à un PAT unique (= 462 lignes).

Les bases de données contiennent des variables dont nous essayerons de comprendre l'intérêt pour la qualification des PAT. 
...
=> le niveau "concertation" devrat correspond aux projets "en attente",  pourtant on peut voir qu'un correspond à une "labellisation niveau 1".
...
On voulait réaliser un graphique de la distribution des PAT selon leur dernière étape d'avancement réalisée, en fonction de leur niveau, afin de voir à quelles étapes se situent les PAT de niveau 1 et de niveau 2.
...
On a cherché à comprendre au sein du fichier de description des PAT, pourquoi chacune des variables a été demandée dans le questionnaire

On a : 
- étudié les taux de NA pour chaque variable 
- cherché à comprendre quelles variables pourraient être sélectionnées comme variables illustratives pour qualifier nos PAT 
- chercher à comprendre quelle dimension de variabilité se cache derrière chacune de ces variables sélectionnées. 

La liste des variables du fichier "description" se trouve dans le fichier "modele de données" dans la base France PAT. Ce fichier n'est pas à jour, les variables ayant évolué depuis 2024. 

Voici les variables qui manquaient : 
Nombre de communes du territoires 
Sau en hectare 
...
![Texte Alternatif](images/variables_illus.png)

# Description des PAT

La colonne "description_libre_des_enjeux_par_le_porteur" est une variable très intéressante, dans laquelle le porteur de projet résume les enjeux de ce projet alimentaire pour son territoire. 
Cette donnée textuelle caractérise donc les PAT. Elle porte un facteur de variabilité individuelle puisque les PAT sont décrits par leur porteur de projet. 

Cette donnée sera mise en lien avec d'autres données textuelles, notamment les variables "objectifs_strategiques" afin de comprendre en quelles proportions les enjeux développés sont traduits ensuites en objectifs dans le PAT. 

On s'est demandés combien de PAT n'avaient pas de description. 
On s'est ensuite demandés combien de PAT n'avaient pas de description selon leur niveau de labellisation.
...
Il y a t-il un lien entre les ‘descriptions_des_enjeux_par_le_porteur’ et les ‘objectifs_strategiques’ ? Peut-on associer ces descriptions aux objectifs en faisant du matching de texte ? (correspondance entre enjeux et obj en %) 

# Objectif stratégiques des PAT 

Est-il intéressant de garder les 5 colonnes de ‘objectifs_strategiques’ ? 
Comment la réponse a cette question est-elle interprétée (différemment?) par les porteurs de projet?
Certains porteurs ont tout mis dans la 1e colonnes, tous les PAT n’ont pas le même nombre d’objectifs (3, 4, 5, 6 ….) 

```{r}
pat2025[26,c(146:150)] # exemple d'un remplissage "normal"
pat2025[44,c(146:150)] # exemple d'un remplissage "tout dans la même case"
pat2025[27, c(146:150)] # exemple d'un remplissage "< 5 axes"

```
Il y a t-il un ordonnancement des objectifs stratégiques de 1 à 5, du plus vers le moins important ? 



# Engagement des PAT dans leurs thématiques

Chaque PAT décide de s'impliquer ou non dans chaque axe thématique et chaque axe thématique est défini par un certain nombre d'actions. Il y a au total 44 actions identifiés sur tous les axes thématiques. 

Chaque PAT réalise entre 0 et 5 actions pour chaque action de chaque axe thématique. 

Comment mettre en valeur les actions réalisées par les PAT pour chaque axe thématique (nom, résultat attendu, résultat obtenu) ? 

On a cherché à comprendre sur quels axes thématiques chaque PAT est le plus impliqué en comptant le nombre d'actions mis en place par chaque PAT (en 2025 sur la déclaration de novembre) pour chaque action de chaque axe thématique. 
...
Voici la répartition des PAT par année de signature. On s'est demandé si elle était représentative du nombre de nouveaux PAT signés chaque année. 



# Que peut on faire de la colonne "methodes_devaluations_1" ? Il y a t-il des indicateurs / mesures de réussite du PAT ? 

“Pour les PAT candidats au niveau 2, il y a selon les directives du ministère de l'Agriculture et de la Souveraineté Alimentaire[6], les candidats doivent élaborer une feuille de route annexée au pland'actions du PAT, s'engageant à suivre au moins un indicateur d’évaluation pour chaque thématique de la Stratégie Nationale Alimentation, Nutrition et Climat (SNANC) et à fixer un objectif quantitatif par indicateur sélectionné.

=> ces indicateurs ne sont pas obligatoirement définis dans la catégorie “mesures d’évaluation” ce qui rend difficile la comparaison des PAT entre eux 
=> pas d’accès à leurs objectifs et la quantification (résultats du PAT) 

C'est une faiblesse importante de la base. 

On peut cependant utiliser les "resultats atteints" et "resultats attendus" de chaque action.


# Comment mesure t - on le niveau de “difficulté” des objectifs fixés par un PAT ? 
# Peut-on représenter une évolution temporelle des PAT au cours du temps ?
# Peut on représenter une distribution du niveau d’avancement des PAT en fonction de la date_de_signature_de_la_convention’ => année de démarrage du PAT ? 
