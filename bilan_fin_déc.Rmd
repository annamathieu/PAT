...
coord.pat <- data.frame(dim1 = res.pca$rowcoord[,1], dim2 = res.pca$rowcoord[,2])
rownames(coord.pat) = rownames(df.t)

res.hcpc <- HCPC(coord.pat, nb.clust = 4)


?easyCODA-package::WARD
```



```{r}
res.pca <- easyCODA::PCA(res.lra$N)
res.lra$
