...
lda.fit <- function (k = k, alpha = 50/k) {
  set.seed(1234)
  lda_model <- LDA(stm, k = k, method = "Gibbs", control = list(alpha =alpha))
...
(le set seed ne fonction pas très bien donc les sujets sont bousculés à chaque nouveau modèle donc ce graphique n'est pas forcément interprétable en l'état mais c'est une idée de résultat que l'on peut obtenir ) : voici ci dessous une capture d'écran du même code exécuté *avec les bonnes légendes*
